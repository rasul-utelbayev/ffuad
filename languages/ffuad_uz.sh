...
usage() {
  cat <<EOF
${YELLOW}FFUAD - Tez Fayl Yuklash va Yuklab Olish${NC}
${BLUE}${VERSION}${NC}
  ...
  ffuad --list                 Tarixni ko'rish
  ffuad --version              Versiyani ko'rish
EOF
  exit 0
}
...
upload() {
  ...
  local code="${url##*/}"
  log "YUKLASH: $(basename "$file") -> $url $([ -n "$pass" ] && echo '[shifrlangan]')"
...
download() {
  ...
  log "YUKLAB OLISH: $code $([ -n "$pass" ] && echo '[shifr ochildi]')"
